...
export default function HomeScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      async function loadDashboardData() {
        ...
        // 1. Fetch ALL Cloud Data from Supabase
        ...
        if (user && user.user_metadata) {
          ...
          // --- 2. CLOUD ROLLOVER LOGIC ---
          const todayStr = getTodayString();
          const cloudIntake = meta.daily_intake; 
          
          if (cloudIntake) {
            if (cloudIntake.date === todayStr) {
              // It's still today! Load the water from the cloud.
              ...
              // IT'S A NEW DAY! Save yesterday to Cloud History
              ...
              historyArray.push({
                day: dayString,
                amount: cloudIntake.amount,
                target: cloudIntake.target || 2000, 
                isMetric: cloudIntake.isMetric
              });
              ...
              // Push the rollover to Supabase!
              await supabase.auth.updateUser({ 
                data: { weekly_history: historyArray, daily_intake: resetIntake } 
              });
              
              setCurrentIntake(0);
            }
          } else {
            // First time ever opening the app: create the cloud folders
  ...
  async function addWater(amount: number) {
    ...
    setCurrentIntake(newIntake); // Fast UI update
      ...
      <View style={styles.headerRow}>
        <View style={styles.headerLeft}>
          <TouchableOpacity onPress={() => router.push('/profile')}>
            <View style={styles.avatarPlaceholder}><Text style={styles.avatarText}>{userName.charAt(0).toUpperCase()}</Text></View>
          </TouchableOpacity>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtonRow}>
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: '#FFE5E5' }]} onPress={() => setShowBottleModal(false)}><Text style={[styles.modalBtnText, { color: '#FF4B4B' }]}>Cancel</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: '#7B61FF' }]} onPress={saveBottleSize}><Text style={styles.modalBtnText}>Save</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
